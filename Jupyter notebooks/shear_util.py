...
def convolve_with_PSF(e1,e2,hlr,PSF_FWHM=0,hsm=HLRShearModel(),mode='convolve',return_T2Tpsf=False,illegal_moments_flag=-11): # correct version!
    ...
    if nbm>0: # it never happened for convolution
        print(f"Unable to run the code in the `{mode}` mode for {nbm} galaxies ({100*nbm/nobj:.2f}%) due to the negative determinant of their moments matrices -- but don't worry we will flag their (e1, e2, hlr) as {illegal_moments_flag}")
